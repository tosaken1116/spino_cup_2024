...
type Props = {
  baseUrl: string;
  ChangeUserPosition: (
    payload: RoomSchema.ChangeUserPosition["payload"]
  ) => void;
...
export const useRoomWSClient = (props: Props) => {
    ...
    switch (data.type) {
      case "ChangeUserPosition":
        props.ChangeUserPosition(
          data.payload as RoomSchema.ChangeUserPosition["payload"]
        );
        break;
      case "JoinRoom":
  ...
  const handleChangeCurrentPosition = (
    payload: RoomSchema.ChangeCurrentPosition["payload"]
  ) => {
    connectionRef.current?.send(
      JSON.stringify({ type: "ChangeCurrentPosition", payload: payload })
    );
  };
  const handleChangeCurrentScreen = (
    payload: RoomSchema.ChangeCurrentScreen["payload"]
  ) => {
    connectionRef.current?.send(
      JSON.stringify({ type: "ChangeCurrentScreen", payload: payload })
    );
  };
  useEffect(() => {
