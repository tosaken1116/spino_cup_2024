...
export const ColorPicker: FC<ColorPickerProps> = ({ onChangeColor }) => {
	...
	const hslToRgb = useCallback((h: number, _s: number, _l: number): RGB => {
		const s = _s / 100;
		const l = _l / 100;
	...
	const drawSelectionMarker = useCallback(() => {
		const canvas = canvasRef.current;
		if (!canvas) return;
		const ctx = canvas.getContext("2d");
		if (!ctx) return;
		...
		const markerX =
			width / 2 + Math.cos(angleRad) * (saturation / 100) * radius;
		const markerY =
			height / 2 + Math.sin(angleRad) * (saturation / 100) * radius;
		...
		ctx.fill();
		ctx.stroke();
	}, [drawColorWheel, hue, isDragging, saturation]);
