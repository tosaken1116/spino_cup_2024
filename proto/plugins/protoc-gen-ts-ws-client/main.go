...
func generateWSClient(gen *protogen.Plugin, file *protogen.File) {
	...
	g.P("import { useEffect, useState } from \"react\";")
	...
	g.P("}")
	g.P(fmt.Sprintf("export const %sWSClient = (props:Props) => {", ToUpperCamelCase(packageName)))
	g.P("	const [connection, setConnection] = useState<ReconnectingWebSocket | null>(")
	g.P("		null,")
	g.P("	);")
